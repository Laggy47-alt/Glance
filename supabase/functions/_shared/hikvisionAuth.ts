// Shared Hikvision ISAPI auth helper.
//
// Hikvision ISAPI requires HTTP Digest authentication. We perform a single
// challenge/response: first request goes unauthenticated, the NVR replies with
// 401 + WWW-Authenticate (Digest realm/nonce/qop), then we resend with the
// computed Authorization header.

export type HikvisionInstance = {
  id: string;
  base_url: string;
  auth_username: string | null;
  auth_password: string | null;
  verify_tls?: boolean;
};

export function trimUrl(u: string) { return u.replace(/\/+$/, ""); }

async function md5Hex(input: string): Promise<string> {
  // Web Crypto doesn't include MD5; use a tiny pure-JS MD5.
  return md5(input);
}

// Compact MD5 implementation (public domain, blueimp-md5 style, trimmed).
function md5(str: string): string {
  function rh(n: number) { const hc = "0123456789abcdef"; let s = "", j; for (j = 0; j <= 3; j++) s += hc.charAt((n >> (j*8+4)) & 0x0F) + hc.charAt((n >> (j*8)) & 0x0F); return s; }
  function ad(x: number, y: number) { const l = (x & 0xFFFF) + (y & 0xFFFF); const m = (x >> 16) + (y >> 16) + (l >> 16); return (m << 16) | (l & 0xFFFF); }
  function rl(n: number, c: number) { return (n << c) | (n >>> (32 - c)); }
  function cm(q: number, a: number, b: number, x: number, s: number, t: number) { return ad(rl(ad(ad(a, q), ad(x, t)), s), b); }
  function ff(a: number, b: number, c: number, d: number, x: number, s: number, t: number) { return cm((b & c) | ((~b) & d), a, b, x, s, t); }
  function gg(a: number, b: number, c: number, d: number, x: number, s: number, t: number) { return cm((b & d) | (c & (~d)), a, b, x, s, t); }
  function hh(a: number, b: number, c: number, d: number, x: number, s: number, t: number) { return cm(b ^ c ^ d, a, b, x, s, t); }
  function ii(a: number, b: number, c: number, d: number, x: number, s: number, t: number) { return cm(c ^ (b | (~d)), a, b, x, s, t); }
  function c2b(s: string) {
    const bytes: number[] = [];
    for (let i = 0; i < s.length; i++) {
      const c = s.charCodeAt(i);
      if (c < 0x80) bytes.push(c);
      else if (c < 0x800) bytes.push(0xC0 | (c >> 6), 0x80 | (c & 0x3F));
      else bytes.push(0xE0 | (c >> 12), 0x80 | ((c >> 6) & 0x3F), 0x80 | (c & 0x3F));
    }
    const nblk = ((bytes.length + 8) >> 6) + 1;
    const blks = new Array(nblk * 16).fill(0);
    let i;
    for (i = 0; i < bytes.length; i++) blks[i >> 2] |= bytes[i] << ((i % 4) * 8);
    blks[i >> 2] |= 0x80 << ((i % 4) * 8);
    blks[nblk * 16 - 2] = bytes.length * 8;
    return blks;
  }
  const x = c2b(str);
  let a = 1732584193, b = -271733879, c = -1732584194, d = 271733878;
  for (let i = 0; i < x.length; i += 16) {
    const oa = a, ob = b, oc = c, od = d;
    a = ff(a, b, c, d, x[i+ 0], 7, -680876936); d = ff(d, a, b, c, x[i+ 1], 12, -389564586); c = ff(c, d, a, b, x[i+ 2], 17,  606105819); b = ff(b, c, d, a, x[i+ 3], 22, -1044525330);
    a = ff(a, b, c, d, x[i+ 4], 7, -176418897); d = ff(d, a, b, c, x[i+ 5], 12,  1200080426); c = ff(c, d, a, b, x[i+ 6], 17, -1473231341); b = ff(b, c, d, a, x[i+ 7], 22, -45705983);
    a = ff(a, b, c, d, x[i+ 8], 7,  1770035416); d = ff(d, a, b, c, x[i+ 9], 12, -1958414417); c = ff(c, d, a, b, x[i+10], 17, -42063); b = ff(b, c, d, a, x[i+11], 22, -1990404162);
    a = ff(a, b, c, d, x[i+12], 7,  1804603682); d = ff(d, a, b, c, x[i+13], 12, -40341101); c = ff(c, d, a, b, x[i+14], 17, -1502002290); b = ff(b, c, d, a, x[i+15], 22,  1236535329);
    a = gg(a, b, c, d, x[i+ 1], 5, -165796510); d = gg(d, a, b, c, x[i+ 6], 9, -1069501632); c = gg(c, d, a, b, x[i+11], 14,  643717713); b = gg(b, c, d, a, x[i+ 0], 20, -373897302);
    a = gg(a, b, c, d, x[i+ 5], 5, -701558691); d = gg(d, a, b, c, x[i+10], 9,  38016083); c = gg(c, d, a, b, x[i+15], 14, -660478335); b = gg(b, c, d, a, x[i+ 4], 20, -405537848);
    a = gg(a, b, c, d, x[i+ 9], 5,  568446438); d = gg(d, a, b, c, x[i+14], 9, -1019803690); c = gg(c, d, a, b, x[i+ 3], 14, -187363961); b = gg(b, c, d, a, x[i+ 8], 20,  1163531501);
    a = gg(a, b, c, d, x[i+13], 5, -1444681467); d = gg(d, a, b, c, x[i+ 2], 9, -51403784); c = gg(c, d, a, b, x[i+ 7], 14,  1735328473); b = gg(b, c, d, a, x[i+12], 20, -1926607734);
    a = hh(a, b, c, d, x[i+ 5], 4, -378558); d = hh(d, a, b, c, x[i+ 8], 11, -2022574463); c = hh(c, d, a, b, x[i+11], 16,  1839030562); b = hh(b, c, d, a, x[i+14], 23, -35309556);
    a = hh(a, b, c, d, x[i+ 1], 4, -1530992060); d = hh(d, a, b, c, x[i+ 4], 11,  1272893353); c = hh(c, d, a, b, x[i+ 7], 16, -155497632); b = hh(b, c, d, a, x[i+10], 23, -1094730640);
    a = hh(a, b, c, d, x[i+13], 4,  681279174); d = hh(d, a, b, c, x[i+ 0], 11, -358537222); c = hh(c, d, a, b, x[i+ 3], 16, -722521979); b = hh(b, c, d, a, x[i+ 6], 23,  76029189);
    a = hh(a, b, c, d, x[i+ 9], 4, -640364487); d = hh(d, a, b, c, x[i+12], 11, -421815835); c = hh(c, d, a, b, x[i+15], 16,  530742520); b = hh(b, c, d, a, x[i+ 2], 23, -995338651);
    a = ii(a, b, c, d, x[i+ 0], 6, -198630844); d = ii(d, a, b, c, x[i+ 7], 10,  1126891415); c = ii(c, d, a, b, x[i+14], 15, -1416354905); b = ii(b, c, d, a, x[i+ 5], 21, -57434055);
    a = ii(a, b, c, d, x[i+12], 6,  1700485571); d = ii(d, a, b, c, x[i+ 3], 10, -1894986606); c = ii(c, d, a, b, x[i+10], 15, -1051523); b = ii(b, c, d, a, x[i+ 1], 21, -2054922799);
    a = ii(a, b, c, d, x[i+ 8], 6,  1873313359); d = ii(d, a, b, c, x[i+15], 10, -30611744); c = ii(c, d, a, b, x[i+ 6], 15, -1560198380); b = ii(b, c, d, a, x[i+13], 21,  1309151649);
    a = ii(a, b, c, d, x[i+ 4], 6, -145523070); d = ii(d, a, b, c, x[i+11], 10, -1120210379); c = ii(c, d, a, b, x[i+ 2], 15,  718787259); b = ii(b, c, d, a, x[i+ 9], 21, -343485551);
    a = ad(a, oa); b = ad(b, ob); c = ad(c, oc); d = ad(d, od);
  }
  return rh(a) + rh(b) + rh(c) + rh(d);
}

function parseChallenge(header: string): Record<string, string> {
  const out: Record<string, string> = {};
  // Strip "Digest "
  const body = header.replace(/^Digest\s+/i, "");
  const re = /(\w+)\s*=\s*(?:"([^"]*)"|([^\s,]+))/g;
  let m: RegExpExecArray | null;
  while ((m = re.exec(body)) !== null) out[m[1].toLowerCase()] = m[2] ?? m[3];
  return out;
}

function randomCnonce() {
  const arr = new Uint8Array(8);
  crypto.getRandomValues(arr);
  return Array.from(arr).map((b) => b.toString(16).padStart(2, "0")).join("");
}

async function buildDigestHeader(
  method: string,
  uri: string,
  username: string,
  password: string,
  challenge: Record<string, string>,
  nc: string,
  cnonce: string,
): Promise<string> {
  const realm = challenge.realm ?? "";
  const nonce = challenge.nonce ?? "";
  const qop = (challenge.qop ?? "").split(",")[0]?.trim() || "";
  const algorithm = (challenge.algorithm ?? "MD5").toUpperCase();
  const opaque = challenge.opaque;

  const ha1 = await md5Hex(`${username}:${realm}:${password}`);
  const ha2 = await md5Hex(`${method}:${uri}`);
  const response = qop
    ? await md5Hex(`${ha1}:${nonce}:${nc}:${cnonce}:${qop}:${ha2}`)
    : await md5Hex(`${ha1}:${nonce}:${ha2}`);

  const parts = [
    `username="${username}"`,
    `realm="${realm}"`,
    `nonce="${nonce}"`,
    `uri="${uri}"`,
    `algorithm=${algorithm}`,
    `response="${response}"`,
  ];
  if (qop) {
    parts.push(`qop=${qop}`);
    parts.push(`nc=${nc}`);
    parts.push(`cnonce="${cnonce}"`);
  }
  if (opaque) parts.push(`opaque="${opaque}"`);
  return `Digest ${parts.join(", ")}`;
}

/**
 * Fetch an ISAPI endpoint with HTTP Digest auth. Performs one unauth attempt
 * then one authenticated retry on 401. Returns the upstream Response (status
 * preserved so callers can branch on 200 vs error).
 */
export async function hikvisionFetch(
  inst: HikvisionInstance,
  path: string,
  init: RequestInit = {},
  timeoutMs = 8000,
): Promise<Response> {
  const base = trimUrl(inst.base_url);
  const url = `${base}${path.startsWith("/") ? path : "/" + path}`;
  const method = (init.method ?? "GET").toUpperCase();
  const uri = new URL(url).pathname + new URL(url).search;

  const doFetch = (extra: Record<string, string> = {}) =>
    fetch(url, {
      ...init,
      method,
      headers: { ...(init.headers as Record<string, string> | undefined ?? {}), ...extra },
      signal: AbortSignal.timeout(timeoutMs),
    });

  // First attempt — if no creds, just go.
  if (!inst.auth_username || !inst.auth_password) return doFetch();

  const first = await doFetch();
  if (first.status !== 401) return first;
  const wa = first.headers.get("www-authenticate") || first.headers.get("WWW-Authenticate");
  if (!wa || !/digest/i.test(wa)) return first;
  // Drain body so the connection can be reused.
  try { await first.arrayBuffer(); } catch { /* ignore */ }

  const challenge = parseChallenge(wa);
  const nc = "00000001";
  const cnonce = randomCnonce();
  const authHeader = await buildDigestHeader(method, uri, inst.auth_username, inst.auth_password, challenge, nc, cnonce);
  return doFetch({ Authorization: authHeader });
}
