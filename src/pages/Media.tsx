...
const Media = () => {
  ...
  const items = useMemo(() => {
    return store.media
      ...
      .filter((m) => !filter || m.camera.includes(filter) || m.topic.includes(filter))
      .slice()
      .sort((a, b) => b.ts - a.ts);
  ...
  return (
    <DashboardLayout
      title="Media"
      subtitle="Every snapshot and clip captured from MQTT"
      actions={
    ...
    >
      ...
      {items.length === 0 ? (
        <Card className="bg-gradient-card border-border shadow-card p-12 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">Snapshots and clips arriving via MQTT will appear here.</p>
        </Card>
      ) : (
          ...
          {items.map((m) => (
            ...
            >
              {m.kind === "snapshot" ? (
                ...
                <>
                  {m.thumbnail ? (
                    <img src={m.thumbnail} alt={m.camera} className="w-full h-full object-cover opacity-80" loading="lazy" />
                  ) : (
                    <div className="grid place-items-center h-full text-muted-foreground"><Film className="h-6 w-6" /></div>
                  )}
