...
function parseStats(stats: unknown): CameraStatus[] {
  ...
  for (const [name, val] of Object.entries(cameras)) {
    ...
    // Frigate camera entries usually have "camera_fps" and "process_fps"/"pid"
    ...
    // Offline heuristic: pid missing/-1 OR camera_fps == 0
    ...
    >
      {enabled.length === 0 ? (
        <Card className="bg-gradient-card border-border shadow-card p-12 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">Add and enable an instance from the Frigate NVR page.</p>
        </Card>
      ) : (
